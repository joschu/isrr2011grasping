...
for i_surf = 1:n_surf
    ...
    R = 6;    
    c_12 = mean(wcont_12n,2);    
    [u,s,v] = svd(bsxfun(@minus,wcont_12n,c_12),'econ');
    B = u*s;
    A = B^-1;
    A_r12 = A(1:R,:);
       
%     G_12q = A_1212 * wres_12q;
    G_rp = A_r12*wsurf_12p;
    G_rn = A_r12*wcont_12n;
    c_r = (A_r12*c_12)';
    
%     t_12k = sphere_samples(2,12)';
    t_rk = sphere_samples(10000,R)';
    
             

    cdots_k = c_r * t_rk;
    pdots_nk = max(G_rn'*t_rk,0);
    pdots_pk = max(G_rp'*t_rk,0);
